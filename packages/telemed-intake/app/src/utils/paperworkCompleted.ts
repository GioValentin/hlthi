...
import { FileURLs, PaperworkPage, Question } from 'ottehr-utils';
...
function isAdditionalComplete(completedPaperwork: CompletedPaperwork, questions: Question[]): boolean {
  return questions.every((question) => {
    return valueExists(completedPaperwork[question.id]);
  });
}
...
function isAllergiesComplete(completedPaperwork: CompletedPaperwork, questions: Question[]): boolean {
  return questions.every((question) => {
    return valueExists(completedPaperwork[question.id]);
  });
}

function isMedicalHistoryComplete(completedPaperwork: CompletedPaperwork, questions: Question[]): boolean {
  return questions.every((question) => {
    return valueExists(completedPaperwork[question.id]);
  });
}

function isSurgicalHistoryComplete(completedPaperwork: CompletedPaperwork, questions: Question[]): boolean {
  return questions.every((question) => {
    return valueExists(completedPaperwork[question.id]);
  });
}
...
export function isPaperworkPageComplete(
  ...
  switch (paperworkPage.slug) {
    ...
    case 'allergies':
      return isAllergiesComplete(completedPaperwork, questions);
    case 'medical-history':
      return isMedicalHistoryComplete(completedPaperwork, questions);
