...
import { getZapEHRAPI, useZambdaClient,useZambdaClientNoParam } from 'ottehr-components';
    ...
    const client = getZapEHRAPI(
      {
        ...
        getBillingPortalZambdaID: import.meta.env.VITE_APP_GET_BILLING_PORTAL_ZAMBDA_ID,
        getStripeCustomerZambdaID: import.meta.env.VITE_APP_GET_STRIPE_CUSTOMER_ZAMBDA_ID,
...
let _apiClientNoParam: ReturnType<typeof getZapEHRAPI> | null;

export const useZapEHRAPIClientNoParams = (
  {
    tokenless,
  }: {
    tokenless: boolean;
  ...
  const zambdaClient = useZambdaClientNoParam({ tokenless });
  const [apiClient, setApiClient] = useState<typeof _apiClientNoParam>(_apiClientNoParam);
