...
import { getVideoRoomResourceExtension,getChatRoomResourceExtension } from '../../shared/helpers';
...
export const getChatResources = async (
  fhirClient: FhirClient,
  appointmentId: string,
): Promise<AppointmentPackage | undefined> => {
  //
  ...
  const encounter: Encounter | undefined = items.find((item: Resource) => {
    return item.resourceType === 'Encounter' && getChatRoomResourceExtension(item);
  }) as Encounter;
  if (!encounter) return undefined;
